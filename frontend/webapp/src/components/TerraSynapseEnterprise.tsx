...
import { Camera, Cloud, Droplets, TrendingUp, AlertTriangle, MapPin, Settings, User, Menu, Bell, Search, Calendar, Download, Zap, Leaf, DollarSign, Activity, Sun, Wind, Thermometer, Eye, Shield, ChevronRight, Play } from 'lucide-react';

const TerraSynapseEnterprise = () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md border border-green-100">
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-gray-600">Primeira vez? <span className="text-green-600 font-semibold cursor-pointer hover:underline">Criar conta enterprise</span></p>
        </div>
      </div>
    </div>
  );

  const MetricCard = ({ icon: Icon, title, value, delta, color = "green" }) => (
    <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:scale-105">
      <div className="flex items-center justify-between mb-4">
        <div className={`p-3 rounded-xl bg-gradient-to-r ${color === 'green' ? 'from-green-100 to-green-200' : color === 'blue' ? 'from-blue-100 to-blue-200' : color === 'orange' ? 'from-orange-100 to-orange-200' : 'from-purple-100 to-purple-200'}`}>
          <Icon className={`w-6 h-6 ${color === 'green' ? 'text-green-600' : color === 'blue' ? 'text-blue-600' : color === 'orange' ? 'text-orange-600' : 'text-purple-600'}`} />
        </div>
        <div className="text-right">
          <p className="text-sm text-gray-500 font-medium">{title}</p>
          <p className="text-2xl font-bold text-gray-800">{value}</p>
          {delta && (
            <p className={`text-sm font-semibold ${delta.startsWith('+') ? 'text-green-600' : delta.startsWith('-') ? 'text-red-600' : 'text-gray-600'}`}>
              {delta}
            </p>
          )}
        </div>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-green-600 to-green-700 rounded-3xl p-8 text-white">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-4xl font-bold">{currentTime.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard 
          ...
          delta={weatherData?.et0 > 5 ? "Irrigação recomendada" : "Normal"}
          color="blue"
        />
        <MetricCard 
          icon={Leaf} 
            ...
            {commodityData.map((commodity, index) => (
              <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-all duration-200">
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full`} style={{ backgroundColor: commodity.color }}></div>
                  <span className="font-semibold text-gray-800">{commodity.name}</span>
                </div>
                <div className="text-right">
                  <div className="text-lg font-bold text-gray-800">R$ {commodity.price.toFixed(2)}</div>
                  <div className={`text-sm font-semibold ${commodity.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
