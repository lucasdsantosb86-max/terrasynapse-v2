// src/components/TerraSynapseEnterprise.tsx
...
// API client (ajuste o caminho se necessário)
import {
  login as apiLogin,
...
const TerraSynapseEnterprise: React.FC = () => {
  ...
  async function handleLogin() {
    try {
      ...
      const token =
        (data as any).access_token || (data as any).token; // compat
  ...
  async function handleRegister() {
    try {
      ...
      await handleLogin(); // auto-login após cadastro
    } catch (e: any) {
  ...
  // Relógio e mocks iniciais (para não ficar vazio antes do fetch real)
  useEffect(() => {
    ...
    setAlerts([
      {
        id: 1,
        type: "warning",
        message: "ET0 elevada detectada - considere irrigação",
        priority: "medium",
        time: "14:30",
      },
      {
        id: 2,
        type: "info",
        message: "Condições ideais para aplicação foliar",
        priority: "low",
        time: "13:45",
      },
    ]);
  ...
  // Buscar dados reais do backend após login
  useEffect(() => {
    if (!isLoggedIn) return;

    (async () => {
      // Default (Ribeirão Preto) se geolocalização indisponível
      let lat = -21.1767,
        lon = -47.8208;
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-green-100 text-lg">
              Monitoramento em Tempo Real • Fazenda São João
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard
          ...
          delta={
            (weatherData?.et0 ?? 0) > 5 ? "Irrigação recomendada" : "Normal"
          }
          color="blue"
        />
        <MetricCard
  ...
  // Renderização
  if (!isLoggedIn) return mode === "login" ? <LoginForm /> : <RegisterForm />;
