...
def hero_path() -> str:
    # por enquanto temos hero dark — funciona bem em tema escuro
...
def obter_localizacao():
    """Descobre localização por IP com fallback para defaults do secrets."""
    try:
...
# =========================================================
def render_topbar():
    # CSS sutil para melhorar visual (não quebra nada)
    st.markdown(
        """
        <style>
...
with st.sidebar:
    ...
    if st.button("🔧 Diagnóstico do Sistema", use_container_width=True):
        ...
        if health:
            st.success("✅ APIs TerraSynapse Online")
            st.json(health)
        else:
            st.error("❌ Sistema Temporariamente Indisponível")

# =========================================================
# Conteúdo (Hero deslogado OU Dashboard logado)
# =========================================================
if not st.session_state.logged_in:
    ...
    with c1:
        if st.button("🔄 Atualizar Dados", type="primary"):
            st.rerun()
    with c2:
        auto_refresh = st.checkbox("⚡ Atualização automática (30s)")
    if auto_refresh:
        time.sleep(30)
        st.rerun()
    ...
    if dash and dash.get("status") == "success":
        ...
        with g2:
            ...
            figb = px.bar(df, x="Commodity", y="Preço", title="Preços Atuais (R$/saca)", color="Preço", color_continuous_scale="Greens")
