...
def calculate_et0(temp_max, temp_min, humidity, wind_speed, solar_radiation=None):
    try:
        ...
        et0 = (0.0023 * (temp_mean + 17.8) * math.sqrt(abs(temp_max - temp_min)) * solar_radiation) / 2.45
        return round(et0, 2)
    except:
        return 4.5
...
async def get_market_data():
    ...
    commodities = {
        "soja": {
            "preco": round(random.uniform(150, 180), 2),
            "variacao": round(random.uniform(-5, 5), 2),
            "tendencia": "Alta" if random.choice([True, False]) else "Baixa"
        },
        "milho": {
            "preco": round(random.uniform(70, 90), 2),
            "variacao": round(random.uniform(-3, 3), 2),
            "tendencia": "Estável" if random.choice([True, False]) else "Baixa"
        },
        "cafe": {
            "preco": round(random.uniform(800, 1200), 2),
            "variacao": round(random.uniform(-8, 8), 2),
            "tendencia": "Alta" if random.choice([True, False]) else "Estável"
        }
    }
...
async def get_satellite(lat: float, lon: float, user: dict = Depends(verify_token)):
    try:
        ndvi_data = await get_ndvi_data(lat, lon)
...
async def get_dashboard(lat: float, lon: float, user: dict = Depends(verify_token)):
    try:
        ...
        produtividade_estimada = 50 if ndvi_data["ndvi"] > 0.6 else 35
